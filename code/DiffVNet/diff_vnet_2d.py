...
class DenoiseModel(nn.Module):
    def __init__(self, n_classes, n_channels, n_filters, normalization, has_dropout, dropout_rate):
        super().__init__()
        self.embedding_diffusion = TimeStepEmbedding(3+n_channels, n_filters, normalization=normalization)
...
class DiffVNet(nn.Module):
    ...
    def forward(self, image=None, x=None, pred_type=None, step=None):
        ...
        if pred_type == "q_sample":
            x_list = x
            t, weight = self.sampler.sample(x_list[0].shape[0], x_list[0].device)
            
            noisy_x_list = []
            noise_list = []

            for x in x_list:
                noise = torch.randn_like(x.float()).to(x.device)
                x_t = self.diffusion.q_sample(x, t, noise=noise)
                noisy_x_list.append(x_t)
                noise_list.append(noise)
            
            x_t = torch.cat(noisy_x_list, dim=1)  # optional: concat for UNet
